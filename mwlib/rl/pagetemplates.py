...
class WikiPage(PageTemplate):
    ...
    def __init__(self,
                 ...
                 pagesize=(pdfstyles.page_width, pdfstyles.page_height),
                 rtl=False,
                 print_width=pdfstyles.print_width,
                 print_height=pdfstyles.print_height
                 ):
        """
        @type title: unicode
        """
    ...
    def beforeDrawPage(self,canvas,doc):
        ...
        #header
        canvas.line(header_margin_hor, self.page_height - header_margin_vert, self.page_width - header_margin_hor, self.page_height - header_margin_vert )
        if pdfstyles.show_page_header:
            ...
            canvas.translate(h_offset, self.page_height - header_margin_vert - 0.1*cm)
            p = Paragraph(self.title, text_style())
            p.canv = canvas
            p.wrap(self.page_width - header_margin_hor*2.5, self.page_height) # add an extra 0.5 margin to have enough space for page number
            p.drawPara()
            canvas.restoreState()

        if not self.rtl:
            h_pos =  self.page_width - header_margin_hor
            d = canvas.drawRightString
        else:
            h_pos = header_margin_hor
            d = canvas.drawString
        d(h_pos, self.page_height - header_margin_vert + 0.1 * cm, "%d" % doc.page)

        #Footer
        canvas.saveState()
        canvas.setFont(serif_font,8)
        canvas.line(footer_margin_hor, footer_margin_vert, self.page_width - footer_margin_hor, footer_margin_vert )
        if pdfstyles.show_page_footer:
            p = Paragraph(formatter.cleanText(pagefooter, escape=False), text_style())
            p.canv = canvas
            w,h = p.wrap(self.page_width - header_margin_hor*2.5, self.page_height)
...
class TitlePage(PageTemplate):
    ...
    def __init__(self, cover=None, id=None,
        onPage=_doNothing, onPageEnd=_doNothing, pagesize=(pdfstyles.page_width, pdfstyles.page_height), print_width=pdfstyles.print_width, print_height=pdfstyles.print_height):
        ...
        p = pdfstyles
        p.page_width = self.page_width
        p.page_height = self.page_height
    ...
    def _scale_img(self, img_area_size, img_fn):
        ...
        img_area_width = min(self.page_width,img_area_size[0])
        img_area_height = min(self.page_height, img_area_size[1])
    ...
    def beforeDrawPage(self,canvas,doc):
        ...
        if pdfstyles.show_title_page_footer:
            ...
            w,h = p.wrap(self.print_width, self.print_height)
            canvas.translate( (self.page_width-w)/2.0, footer_margin_vert - h - 0.25*cm)
            p.canv = canvas
            p.draw()
        canvas.restoreState()
        if self.cover:
            ...
            if pdfstyles.title_page_image_pos[0] == None:
                x = (self.page_width - width) / 2.0
            else:
                x = max(0, min(self.page_width-width, pdfstyles.title_page_image_pos[0]))
            if pdfstyles.title_page_image_pos[1] == None:
                y = (self.page_height - height) / 2.0
            else:
                y = max(0, min(self.page_height-height, pdfstyles.title_page_image_pos[1]))
